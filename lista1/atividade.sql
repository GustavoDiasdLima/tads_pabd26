...
DROP TABLE IF EXISTS orders_products CASCADE; --itens que compõem cada pedido.
DROP TABLE IF EXISTS orders     CASCADE; --pedidos realizados pelos usuários.
DROP TABLE IF EXISTS products   CASCADE; -- produtos 
...
from products
order by price desc;
--.Aumente o preço de todos os produtos da Dell em 10%. Notebook Dell Inspiron e Monitor 27" Dell'
update products 
set price = price *1.10 
where id in (1, 4); 
--4.Exclua todos os produtos que sejam do tipo Macbook. id 9 Notebook Apple Macbook Pro M
delete from products
where id = 9
returning id, name, price;
--5.Exclua um produto que não possua pedidos associados. produto de id 9? ele já foi excluido na questão 4 :/
delete from products
where id = 9
returning id, name, price; -- usa returning em delete, insert e update
--6.Liste todos os pedidos realizados nos últimos 30 dias. tabela orders, order date, interval: year, month, day. Possibilidade de uso: '1 year, 1 month, 12 days'
select * from orders
where order_date >= now() - interval '30 days';
--7.Liste os pedidos e os respectivos nomes de usuário. são todos os produtos ou da questão 6 ???
select orders.id, users.name -- tabela orders e users vai retornar o id dos pedidos + nome dos usuarios
from orders
